...
## CMS-0.3.7.10 更新日志
1. 优化自动整理逻辑，转存文件夹现在可以不和待整理文件夹设置同一个
2. 增加转存后是否整理开关
3. 冗余文件夹可以不设置（整理后不会清空待整理，但是有风控的几率，不建议为空）
4. 增强自动整理识别策略

## CMS-0.3.7.9 更新日志
1. 优化自动整理逻辑，转存文件夹现在可以不和待整理文件夹设置同一个（仍会自动整理）
2. 增强自动整理识别策略

## CMS-0.3.7.7-0.3.7.8 更新日志
1. 修复移动文件失败的问题
2. 增强自动整理识别策略

## CMS-0.3.7.4-0.3.7.6 更新日志
1. 快速同步支持同步音乐
2. 修复openapi回收站无法清空的问题
3. 修复部分电影会创建Season 01文件夹的问题
4. 增强自动整理识别策略

## CMS-0.3.7.1-0.3.7.3 更新日志
1. 修复openapi图片下载的问题
2. 修复使用openapi时部分视频无法播放的问题
3. 增强自动整理识别策略

## CMS-0.3.7.0 更新日志
1. 支持115 openapi（需要自己准备AppID）（100196XXX）
看到以下日志内容代表启用成功

## CMS-0.3.6.10 更新日志
1. 优化自动整理识别策略，待整理文件夹名称如果是中文请包含 “整理” 关键字
2. 二级分类策略 year 支持2000-2024范围匹配

## CMS-0.3.6.9 更新日志
1. 优化自动整理，增强识别，修复剧集整理S00会整理为S01的问题
2. 优化日志分隔策略
...
## CMS-0.3.6.5 更新日志
1. 优化洗版逻辑，修复共存不起作用
2. 优化订阅，如果分享是单集，支持保存多个
3. 机器人保存分享支持隐藏链接
4. 优化阿里分享保存逻辑
   
## CMS-0.3.6.3-0.3.6.4 更新日志
1. 自动整理支持自定义洗版策略（洗版是高优先级替换低优先级，请一定的理解这句话，不理解就问下群友）
[#洗版策略](https://github.com/guyue2005/CMSHelp/wiki/6.%E8%87%AA%E5%8A%A8%E6%95%B4%E7%90%86#%E6%B4%97%E7%89%88%E7%AD%96%E7%95%A5)  
2. 频道订阅支持隐藏链接（需要打开小程序或外部网页获取的不行）
3. 修复媒体库封面生成插件定时任务无法关闭的问题
...
## CMS-0.3.5.18 更新日志
1. 优化媒体库封面生成插件，支持自定义字体、大小、背景色、海报位置
2. 优化自动整理，识别更强了

## CMS-0.3.5.17 更新日志
1. 修复上个版本自动整理的BUG
2. 修复媒体库封面生成插件的海报选取策略（需要重新选）
